              ...
              const searchPromises = scope.map(({ file_id, threshold }) => {
                  ...
                  return supabaseAdmin
                      .rpc('match_knowledge', {
                          ...
                          match_count: topK,
                          p_file_id: file_id // Pass file_id to the DB function
                      })
                      .then(result => {
                          if (result.error) {
                          ...
                          // The RPC function now handles the filtering, so we just use the data directly.
                          const directData = result.data || [];
                          
                          console.log(`  ✅ File ${file_id}: ${directData.length} chunks found (from RPC)`);
                          return { 
                              data: directData, 
                              file_id, 
                              threshold 
