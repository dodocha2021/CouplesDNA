...
BEGIN
  ...
  SELECT json_build_object(
    'success', true,
    'message', 'Test trigger executed successfully - check Edge Function logs for processing details',
    'timestamp', now(),
...
-- 第六步：验证所有组件创建成功
SELECT 'All components created successfully! Extensions enabled, trigger and test function ready.' AS status;

-- 第七步：显示可用的扩展（用于验证）
SELECT name, installed_version 
FROM pg_available_extensions 
WHERE name IN ('vector', 'pg_net') 
  AND installed_version IS NOT NULL;

-- 第八步：检查 documents 表结构是否正确
SELECT 
    column_name,
    data_type,
    is_nullable,
...
-- 2. 确保你的 Edge Function 已经部署成功
-- 3. 触发器将在每次向 'chat-logs' bucket 上传文件时自动激活
-- 4. 使用 SELECT test_vector_processing_trigger(); 来测试触发器
-- 5. 检查 Edge Function 日志查看处理结果